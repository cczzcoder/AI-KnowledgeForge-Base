...
export default function GraphPage() {
  ...
  const [subgraphCenterName, setSubgraphCenterName] = useState<string | null>(
    null,
  );
  ...
  const visibleNodeIds = useMemo(() => {
    ...
    const margin =
      150 / Math.max(scale * Math.max(svgToScreenX, svgToScreenY), 0.1);
  ...
  useEffect(() => {
    ...
    const nodeRadiuses = baseNodes.map((n) =>
      calcNodeRadius(nodeDegrees.get(n.id) || 0),
    );
  ...
  return (
    <div className="graph-page">
      <div className="graph-header">
        ...
        <Space>
          {subgraphView && (
            <Button size="small" onClick={handleBackToFullGraph}>
              返回完整图谱
            </Button>
          )}
          <AutoComplete
            ...
            options={searchResults.map((n) => ({
              label: (
                <Space>
                  <Tag
                    color={getEntityColor(n.entityType)}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="entity-card-type">
                      <Tag color={getEntityColor(hoveredNode.entityType)}>
                    ...
                    <div className="entity-card-rel">
                      关联: {hoveredNode.relationCount} 条
                    </div>
                  </div>
                );
              })()}
      ...
      <Drawer
        title={
          <Space>
            ...
            <span>实体详情</span>
            {selectedEntity && (
              <Tag color={getEntityColor(selectedEntity.entityType)}>
        ...
        {detailLoading ? (
          ...
          <div className="entity-detail-content">
            ...
            <div className="entity-detail-section">
              ...
              <div className="entity-detail-value">
                <Tag color={getEntityColor(entityDetail.entityType)}>
            ...
            {entityDetail.relatedEntities &&
              entityDetail.relatedEntities.length > 0 && (
                <>
                  <Divider />
                  <Text strong style={{ fontSize: 14 }}>
                    <LinkOutlined /> 关联实体 (
                    {entityDetail.relatedEntities.length})
                  </Text>
                  <List
                    style={{ marginTop: 12 }}
                    size="small"
                            ...
                            <Space>
                              ...
                              <Text
                                ...
                                onClick={async () => {
                                  ...
                                  if (kbId) {
                                    try {
                                      const res = await getEntityDetail(
                                        item.name,
                                        kbId,
                                      );
                                      if (res.data) {
